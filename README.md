...
- Full-width Omarchy-inspired Waybar with workspaces, date/time, a DST-safe calendar tooltip, temperature, CPU, GPU, volume, battery, and tray. A globally rounded pill interface remains selectable.
- NetworkManager and Blueman tray icons for WiFi and Bluetooth status/changes.
- Wofi launcher, Foot terminal, Dunst notifications, CAVA audio visualizer, and independent theme, font, and global UI style selectors.
- Tokyo Night, Catppuccin Mocha, Gruvbox Dark, Nord, Dracula, and the previous local palettes.
- JetBrainsMono Nerd Font for the interface and terminal, with selectable local fallbacks.
- `keyd` remaps:
...
JetBrainsMono Nerd Font release pinned by
`scripts/install-jetbrainsmono-nerd-font.sh`, installs the
...
- `Super + Shift + F`: font picker.
- `Super + Shift + L`: global UI style picker.
...
Hover the clock to see the monthly calendar tooltip. Click the clock to open a
larger `yad` calendar window. The tooltip is generated by
`uconsole-clock-calendar` instead of Waybar's internal month renderer, avoiding
the invalid-midnight failure caused by Chilean daylight-saving transitions.
...
```sh
uconsole-theme list
uconsole-theme classic
uconsole-theme tokyo-night
uconsole-theme catppuccin-mocha
uconsole-theme gruvbox-dark
uconsole-theme nord
uconsole-theme dracula
uconsole-theme rose
...
The theme script updates Hyprland window borders, Waybar, GTK
tooltip/calendar styling, Wofi, Foot, and Dunst. Themes do not overwrite the
selected font or global UI style. Already-open terminals may need to be reopened
to pick up all colors.

Tokyo Night uses the classic Hyprland combination of gaps, blur, shadows, and
gradient borders. Shape and rounding are controlled independently by the UI
style picker.

## Fonts

JetBrainsMono Nerd Font is installed in the user profile by the full installer.
Install or refresh it independently with:

```sh
./scripts/install-jetbrainsmono-nerd-font.sh
```

List or apply fonts without changing the color theme:

```sh
uconsole-font list
uconsole-font jetbrains
uconsole-font classic
uconsole-font dejavu
uconsole-font liberation
uconsole-font noto
```

Open the font picker with `Super + Shift + F`. The `classic` option restores
the previous Nunito Sans and DejaVu Sans Mono combination.

## UI Styles

The global interface shape is independent from the color theme and font:

```sh
uconsole-ui-style list
uconsole-ui-style omarchy
uconsole-ui-style pills
```

`omarchy` uses a continuous rectangular header, square Hyprland windows, square
Wofi controls, tooltips, calendars, and notifications. `pills` restores the
floating rounded bar modules and rounded interface throughout the desktop.
Open the UI style picker with `Super + Shift + L`.
